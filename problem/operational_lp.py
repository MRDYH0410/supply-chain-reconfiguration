...
def solve_operational_lp(
    ...
    """
    Solve the single-period operational LP.

    Important:
    - keep the project on scipy.optimize.linprog
    - do NOT introduce pulp as a new dependency
    - support both old regime-based tariff lookup and the new time-series tariff lookup
    """
    try:
        ...
        raise ImportError(
            ...
            "Please install scipy, or restore the original scipy-based solver."
        ) from e
    ...
    # outbound cost with tariff
    for j in plants:
        for k in markets:
            c[q_idx[(j, k)]] = scenario.tariff_inclusive_delivered_cost(
                j, k, xi_t, t=t
            )

    # inbound material + inbound logistics cost
    for i in suppliers:
        for j in plants:
            mat = scenario.material_unit_cost(
                i,
                j,
                candidate_age=(age if j == scenario.candidate_plant_id else scenario.H_rel),
            )
    ...
    for j in plants:
        for k in markets:
            C_out += scenario.tariff_inclusive_delivered_cost(j, k, xi_t, t=t) * q[(j, k)]
